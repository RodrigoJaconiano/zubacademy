...
export async function getAdminDashboardData(): Promise<AdminDashboardData> {
  ...
  const [
    ...
    adminSupabase
      ...
      .select("user_id, rating, primary_feedback, secondary_feedback, created_at"),
  ]);
  ...
  const certificatesByUser = new Map<string, number>();

  safeCertificates.forEach(cert => {
    if (!cert.user_id) return;
    certificatesByUser.set(cert.user_id, 1);
  });
  ...
  safeQuizAttempts.forEach(attempt => {
    ...
    const arr = quizByUser.get(attempt.user_id) ?? [];
    arr.push(attempt);
    quizByUser.set(attempt.user_id, arr);
  });

  const latestFeedbackByUser = new Map<string, CertificateFeedbackRow>();

  safeCertificateFeedback.forEach(feedback => {
    if (!feedback.user_id) return;

    if (!latestFeedbackByUser.has(feedback.user_id)) {
      latestFeedbackByUser.set(feedback.user_id, feedback);
    }
  });
  ...
  const adminUsers: AdminUser[] = safeProfiles.map(profile => {
    ...
    const attempts = quizByUser.get(profile.id) ?? [];
    ...
    const latestFeedback =
      latestFeedbackByUser.get(profile.id);
  ...
  const summary: AdminSummary = {
    ...
    completedProfiles:
      adminUsers.filter(u => u.isComplete).length,
    ...
    approvedUsers:
      adminUsers.filter(u => u.quizPassed).length,
    ...
    uniqueCertifiedUsers:
      new Set(
        safeCertificates.map(c => c.user_id)
      ).size,
  ...
  const storeMetrics: AdminStoreMetric[] =
  ...
  return {
    ...
    certificates:
      safeCertificates as AdminCertificate[],
